...
const uint8_t fan_steps[] = {0, 255, 64, 128, 160, 192, 224};
...
class CutoffSelect : public Screen {
public:
  void Draw() override {
    display.setCursor(0, 0);
    display.setTextColor(WHITE);
  ...
  void Select() override {
    settings.cutoff_idx++;
    if (settings.cutoff_idx >= sizeof(cutoff_mv) / sizeof(uint16_t)) {
      settings.cutoff_idx = 0;
    }
    PersistentSettings();
  }
};
...
Screen* settings_screens[] = {&cutoff, &heartbeat, &voltage, &sram,
                              &settings_back};
Rotation settings_rot{settings_screens,
                      sizeof(settings_screens) / sizeof(Screen*)};
